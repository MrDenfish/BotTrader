...
class TradeBotUtils:
    ...
    async def get_open_orders(self, order_data):
        ...
        try:
            ...
            balances = await self.get_account_balance([coin, order_data['quote_currency']])
            ...
            base_value = balances.get(order_data['trading_pair'].split('/')[0], 0)
            base_balance = self.float_to_decimal(base_value, order_data['base_decimal']) if base_value is not None else (
                Decimal(0))
            quote_value = balances.get(order_data['quote_currency'], 0)
            quote_balance = self.float_to_decimal(quote_value, order_data['quote_decimal']) if quote_value is not None else (
                Decimal(0))
            ...
            if all_open_orders.empty or all_open_orders is None:
                ...
                return quote_balance, base_balance, pd.DataFrame()  # Return an empty DataFrame instead of None
    ...
    def adjust_precision(self, base_deci, quote_deci, num_to_adjust, convert):
        ...
        """"" general utility method that can be used throughout your application to ensure any numeric value is adjusted to
