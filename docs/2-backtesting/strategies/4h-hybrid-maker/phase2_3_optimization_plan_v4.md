...
## 10) Frequency Rescue Playbook (When You Get ~0–1 Trades/Year)

If a 365-day baseline run produces **~0–1 signals/trades** (as you observed), you are not in “parameter optimization” territory yet. You must first identify which gate is acting as a kill-switch and then calibrate thresholds so the strategy fires enough to evaluate.

Below are six concrete steps and a recommended next-step sequence.

### 10.1 Add Gate-Audit Counters (stop guessing)
On **every completed 4h bar**, count how often each gate passes:

- `regime_ok`
- `viability_ok`
- `compression_ok` (recent compression)
- `setup_trigger_ok` (Donchian breakout OR ROC-score condition)
- `setup_created`
- `entry_filled`

Report totals and pass rates:

| Gate | Pass count | Pass % of 4h bars |
|------|------------|-------------------|
| regime_ok | ... | ... |
| viability_ok | ... | ... |
| compression_ok | ... | ... |
...
This immediately reveals whether **compression**, **viability**, **regime**, or **setup trigger** is suppressing frequency.

### 10.2 Run “One Gate Off” A/B Tests (identify the kill-switch)
Do three quick 365-day runs:

A) **Compression OFF** (leave regime + viability + setup trigger intact)
- If signals jump materially → compression settings/implementation are the bottleneck.

B) **Viability OFF** (leave compression + regime + setup trigger intact)
- If signals jump → volatility/fee coupling is too strict (or miscalibrated).

C) **Regime relaxed** (EMA200 → EMA100 OR use 4h EMA200)
- If signals jump → daily EMA200 gate is blocking most of the year.

Only after you know the bottleneck should you tune thresholds.

### 10.3 Calibrate ROC-score Threshold by Percentile (don’t guess)
If ROC-score mode produces ~1 signal/year, your `roc_score_thresh` is almost certainly above the realized distribution.

Compute the distribution of `roc_score` on completed 4h bars (optionally conditioned on `regime_ok=True`) and choose thresholds by percentile:

- Start with P80 / P85 / P90 thresholds and observe setup counts/year.

Target frequency math:
- 2–4 trades/month ≈ 24–48 trades/year.
- With missed fills, target **60–120 setups/year** before entry conversion.

Pick `roc_score_thresh` that yields the desired setup rate **before** you tighten gates.

### 10.4 Loosen Compression Filter Parameters (if compression is the bottleneck)
If compression gate pass rate is tiny, try:

- `bb_width_window = 120` (more adaptive)
- `bb_width_pct_threshold = 35` (temporarily, to restore frequency; later tighten)
- `compression_lookback_4h = 12` (2 days of 4h bars; “recent compression”)

Then re-check gate pass rates.

### 10.5 Decouple Viability from Fees (if viability is the bottleneck)
If viability (`atr_pct_4h >= vol_min_mult * fee_rt_est`) suppresses signals:

- temporarily set `vol_min_mult = 0.75` or `1.0`
- optionally add an **absolute ATR floor** independent of fees:
  - `atr_floor_pct = 0.006` (0.6%) to start (BTC-specific; tune later)

Restore stricter viability only after you have adequate samples.

### 10.6 Shorten Donchian Lookback (if setup trigger is the bottleneck)
If raw Donchian breakouts are rare in your year-long window:

- test `donch_len = 6` or `8` (in addition to 10/12)

But do this only after confirming the bottleneck isn’t compression/viability/regime.

### 10.7 Recommended Next Step Sequence (do this now)
1) Implement **gate-audit counters** and print the pass-rate table.
2) Run the three A/B tests:
   - Compression OFF
   - Viability OFF
   - Regime relaxed
3) Calibrate ROC-score threshold by percentile to achieve **60–120 setups/year** pre-fill.
4) Re-enable gates incrementally until you land in the 2–4 trades/month band.
5) Only then proceed with Stage A/Stage B optimization.
...
## 11) Compression Bottleneck Experiment Pack (New)

When gate-audit results show that **Setup Trigger + Compression** eliminates ~98%+ of otherwise viable bars, do *not* proceed directly to large grid search. Instead, run the following targeted experiments to isolate which component is the bottleneck and to restore setup frequency to a level suitable for optimization.

### 11.1 Sanity Check: BB Width Percentile Distribution
Before changing thresholds, confirm the compression signal behaves sensibly.

On all 4h bars that pass `regime_ok AND viability_ok`, compute and print:
- `bb_width_pct` min / median / max
- % of bars with `bb_width_pct <= 20`, `<= 30`, `<= 40`

If these percentages are near zero, suspect percentile windowing/alignment or a bug in the computation.

### 11.2 A/B Test: Compression OFF (Primary discriminator)
Run 365-day tests for both setup modes with **compression disabled** while keeping:
- regime ON
- viability ON
- setup trigger ON

Collect the same gate table metrics.

Interpretation:
- If setups increase from ~1/year to dozens/year → compression gating is the dominant bottleneck.
- If setups remain rare → setup trigger thresholds are too strict (donch_len / roc_score_thresh).

### 11.3 ROC-score Threshold Calibration by Percentile (Compression OFF)
Calibrate `roc_score_thresh` using percentile ranks of `roc_score` over viable bars.

Procedure:
1) Compute `roc_score` distribution for bars that pass `regime_ok AND viability_ok`
2) Choose candidate thresholds at percentiles:
   - P70, P80, P85, P90
3) For each candidate, count setups/year (with compression OFF)

Goal:
- Achieve ~60–120 setups/year pre-fill (to land near 2–4 trades/month after conversion)

Only after finding a viable threshold range should you re-enable compression.

### 11.4 Controlled Loosening of Compression (Compression ON)
If compression is confirmed as the bottleneck, loosen it in a controlled way and re-run gate audits.

Recommended first loosening set:
- `bb_width_window`: 180 → **120**
- `bb_width_pct_threshold`: 20 → **35**
- `compression_lookback_4h`: 6 → **12**
- Keep Phase 2.2 default behavior: **ANY compressed in last K bars**
...
### 11.5 High-Value Bonus: Decompose Setup vs Compression vs Intersection
For each 365-day run, print three counts (on bars that pass regime+viability):

1) `setup_trigger_only_count` (setup trigger ignoring compression)
2) `compression_only_count` (compression_ok ignoring setup trigger)
3) `intersection_count` (setup AND compression)

This reveals whether compression should remain a hard gate or become a soft preference:

- If setup triggers are common but compression is rare, consider making compression a *ranking score* rather than a hard AND gate.
- If compression is common but setup triggers are rare, focus on tuning `donch_len` / `roc_score_thresh`.
- If both are rare, loosen both gates before optimizing exits.

### 11.6 Optional Soft-Gate Variant (If Needed for Frequency)
If intersection remains too small after loosening, implement a soft-gate option:

- `compression_mode = HARD` (current): require compression_ok
- `compression_mode = SOFT`: allow setups without compression, but:
  - require higher `roc_score` (or smaller donch_len), OR
  - apply a penalty/score and only trade top N setups per month

Do not introduce SOFT mode until you have measured the counts in 11.5.

---

## 12) Recommended Next-Step Sequence (Updated)

When you observe the compression bottleneck:

1) Add **bb_width_pct distribution print** (11.1)
2) Run **Compression OFF** A/B test (11.2)
3) Run **ROC-score percentile calibration** with compression OFF (11.3)
4) Re-enable compression with loosened parameters (11.4)
5) Always print setup/compression/intersection counts (11.5)
6) Only then proceed with Stage A/Stage B optimization on 365+ day windows
