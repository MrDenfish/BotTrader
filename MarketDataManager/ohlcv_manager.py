...
class OHLCVManager:
    ...
    async def fetch_last_5min_ohlcv(self, symbol, timeframe='1m', limit=5):
        ...
        Returns:
            DataFrame: OHLCV data (last 5 candles) or None if failed.
        """
        try:
            ...
            if symbol in self.ohlcv_cache:
                ...
                if last_cached_time >= five_min_ago:
                    ...
                    newest_close = cached_data['data'].iloc[-1]['close']  # first row
                    oldest_close = cached_data['data'].iloc[0]['close']  # last row
                    return oldest_close, newest_close

            # ✅ Step 2: Fetch Fresh Data from API
            safe_since = int((five_min_ago - timedelta(seconds=1)).timestamp() * 1000)
            safe_since = self.shared_utiles_data_time.time_sanity_check(safe_since)
            endpoint = 'public'
            params = {'paginate': False}  # No need for pagination in short queries
            ...
            if ohlcv_result and not ohlcv_result['data'].empty:
                ...
                # ✅ Step 3: Resample, Forward-Fill, and Store in Cache
                df['time'] = pd.to_datetime(df['time'], unit='ms')
                df = df.set_index('time')
                df = df.resample('1min').asfreq().ffill().reset_index()  # Fill gaps

                # ✅ Update Cache
                self.ohlcv_cache[symbol] = {"timestamp": now, "data": df}
                newest_close = df.iloc[-1]['close']  # first row
                oldest_close = df.iloc[0]['close']  # last row
