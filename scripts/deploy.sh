#!/bin/bash
#
# Generic deployment script for BotTrader services
# Usage: ./scripts/deploy.sh [service_name] [--verify-only]
#
# Examples:
#   ./scripts/deploy.sh webhook          # Deploy webhook service
#   ./scripts/deploy.sh sighook          # Deploy sighook service
#   ./scripts/deploy.sh report           # Deploy report service
#   ./scripts/deploy.sh all              # Deploy all services
#   ./scripts/deploy.sh webhook --verify-only  # Only verify current version
#

set -e  # Exit on error

# Configuration
REMOTE_HOST="bottrader-aws"
REMOTE_PATH="/opt/bot"
...
# Function to verify remote version
verify_remote_version() {
    local service=$1
    print_info "Checking deployed version of $service..."

    ssh $REMOTE_HOST "docker logs $service --tail 100 2>&1" | grep -E "Version:|Commit:|Branch:" | head -5 || {
...
deploy_service() {
    ...
    print_header "DEPLOYING $service TO AWS"

    # Step 1: Pull latest code on remote
    ...
    ssh $REMOTE_HOST "docker builder prune -f" > /dev/null 2>&1 || {
