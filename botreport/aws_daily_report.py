...
from botreport.data_access import resolve_time_window, assemble_bundle, load_windowed_data
from botreport.compute import compute_metrics
from botreport.render.text_renderer import TextRenderer
from botreport.emailer import send_email
from botreport.config import load_report_dotenv_if_needed, load_report_config
load_report_dotenv_if_needed()
RCFG = load_report_config()
...
#     invested_pct_of_equity: Optional[float] = None  # e.g., 160.46 means 160.46%
#     leverage_used: Optional[float] = None           # e.g., 1.605
#     long_notional: Optional[float] = None
#     short_notional: Optional[float] = None
#     net_exposure_abs: Optional[float] = None        # absolute dollars
#     net_exposure_pct: Optional[float] = None        # e.g., -159.66 means -159.66%
#     positions: Optional[List[PositionRow]] = None   # None or empty -> no open exposure
#
# @dataclass
# class MetricsBlock:
#     as_of_iso: str
#     window_label: str                  # "last 24h" etc.
#     source_label: str                  # e.g. "report_trades"
#     total_trades: Optional[int] = None
#     breakeven_trades: Optional[int] = None
#     win_rate_pct: Optional[float] = None            # 60.4 means 60.4%
...
#     max_drawdown_pct: Optional[float] = None        # -0.25 means -0.25%
...
#         return str(x)
#
# def _fmt_pct(x: Optional[float], signed: bool = False) -> str:
#     try:
#         if x is None:
#             return "n/a"
...
#         return str(x)
#
# def render_text_report(metrics: MetricsBlock, exposure: ExposureBlock, notes: str, csv_note: Optional[str] = None) -> str:
#     # Header
#     lines = []
#     lines.append("Daily Trading Bot Report")
#     lines.append(f"As of: {metrics.as_of_iso} (UTC) · Window: {metrics.window_label} · Source: {metrics.source_label}")
#     lines.append("")
#
#     # Key Metrics (single canonical layout)
#     lines.append("Capital & Exposure")
#     lines.append("Key Metrics")
#     lines.append(" Stat                    | Value")
#     lines.append("----------------------------------------")
#
#     # Always show realized/unrealized if present
#     if metrics.realized_pnl is not None:
#         lines.append(f" Realized PnL (USD)      | {_fmt_money(metrics.realized_pnl)}")
#     if metrics.unrealized_pnl is not None:
#         lines.append(f" Unrealized PnL (USD)    | {_fmt_money(metrics.unrealized_pnl)}")
#
#     # Then the unified stats set
#     if metrics.total_trades is not None:
#         lines.append(f" Total Trades            | {metrics.total_trades}")
#     if metrics.breakeven_trades is not None:
#         lines.append(f" Breakeven Trades        | {metrics.breakeven_trades}")
#     if metrics.win_rate_pct is not None:
#         lines.append(f" Win Rate                | {_fmt_pct(metrics.win_rate_pct)}")
#     if metrics.avg_win is not None:
#         lines.append(f" Avg Win                 | {_fmt_money(metrics.avg_win)}")
#     if metrics.avg_loss is not None:
#         lines.append(f" Avg Loss                | {_fmt_money(metrics.avg_loss)}")
#     if metrics.avg_w_over_avg_l is not None:
#         lines.append(f" Avg W / Avg L           | {_fmt_float(metrics.avg_w_over_avg_l, 3)}")
#     if metrics.profit_factor is not None:
#         lines.append(f" Profit Factor           | {_fmt_float(metrics.profit_factor, 3)}")
#     if metrics.expectancy_per_trade is not None:
#         lines.append(f" Expectancy / Trade      | {_fmt_money(metrics.expectancy_per_trade)}")
#     if metrics.mean_pnl_per_trade is not None:
#         lines.append(f" Mean PnL / Trade        | {_fmt_money(metrics.mean_pnl_per_trade)}")
#     if metrics.stdev_pnl_per_trade is not None:
#         lines.append(f" Stdev PnL / Trade       | {_fmt_money(metrics.stdev_pnl_per_trade)}")
#     if metrics.sharpe_like_per_trade is not None:
#         lines.append(f" Sharpe-like (per trade) | {_fmt_float(metrics.sharpe_like_per_trade, 4)}")
#     if metrics.max_drawdown_pct is not None:
#         if metrics.max_drawdown_abs is not None:
#             lines.append(f" Max Drawdown (window)   | {_fmt_pct(metrics.max_drawdown_pct, signed=True)} ({_fmt_money(metrics.max_drawdown_abs)})")
#         else:
#             lines.append(f" Max Drawdown (window)   | {_fmt_pct(metrics.max_drawdown_pct, signed=True)}")
#
#     lines.append("")
#     lines.append("Capital & Exposure")
#
#     # Exposure block—always print in the same format
#     if not exposure.positions:
#         lines.append("  • No open exposure. Total Notional: " + _fmt_money(exposure.total_notional))
#     else:
#         # Summary fields table
#         def add_field(label: str, val: Optional[str]):
#             if val is not None:
#                 lines.append(f"{label: <22}{val}")
#
#         lines.append("")
#         add_field("Field", "Value")
#         add_field("Total Notional", _fmt_money(exposure.total_notional))
#         add_field("Invested % of Equity", _fmt_pct(exposure.invested_pct_of_equity))
#         add_field("Leverage Used", _fmt_float(exposure.leverage_used, 3) + "×" if exposure.leverage_used is not None else None)
#         add_field("Long Notional", _fmt_money(exposure.long_notional))
#         add_field("Short Notional", _fmt_money(exposure.short_notional))
#         if (exposure.net_exposure_abs is not None) and (exposure.net_exposure_pct is not None):
#             lines.append(f"Net Exposure            {_fmt_money(exposure.net_exposure_abs)} ({_fmt_pct(exposure.net_exposure_pct, signed=True)})")
#         lines.append("")
#
#         # Positions table
...
#
#     # Notes section (definitions, caveats)
#     if notes:
#         lines.append("")
#         lines.append(notes.strip())
#
#     # Optional CSV save note
#     if csv_note:
#         lines.append("")
#         lines.append(f"[Saved CSV] {csv_note}")
#
#     return "\n".join(lines)
# -------------------------
# Config / Environment
...
#     # Map your raw result dict into the dataclasses expected by render_text_report()
#     # 1) ExposureBlock and positions
#     pos_rows = result.get("open_positions") or []
...
#         ))
#
#     exp_data = result.get("exposures") or {}
#     exposure = ExposureBlock(
#         total_notional=float(exp_data.get("total_notional", 0.0)),
#         invested_pct_of_equity=exp_data.get("invested_pct_of_equity"),
#         leverage_used=exp_data.get("leverage_used"),
#         long_notional=exp_data.get("long_notional"),
#         short_notional=exp_data.get("short_notional"),
#         net_exposure_abs=exp_data.get("net_abs"),
#         net_exposure_pct=exp_data.get("net_pct"),
#         positions=positions or None,
#     )
#
#     # 2) MetricsBlock
#     metrics = MetricsBlock(
#         as_of_iso=result["as_of_iso"],
#         window_label=result["window_label"],
#         source_label=result["source_label"],
#         realized_pnl=result.get("realized_pnl"),
#         unrealized_pnl=result.get("unrealized_pnl"),
#         total_trades=result.get("total_trades"),
#         breakeven_trades=result.get("breakeven_trades"),
#         win_rate_pct=result.get("win_rate"),
#         avg_win=result.get("avg_win"),
#         avg_loss=result.get("avg_loss"),
#         avg_w_over_avg_l=(
#             (result["avg_win"] / abs(result["avg_loss"]))
#             if (result.get("avg_win") is not None and result.get("avg_loss") not in (None, 0.0))
#             else None
#         ),
#         profit_factor=result.get("profit_factor"),
#         expectancy_per_trade=result.get("expectancy_per_trade"),
#         mean_pnl_per_trade=result.get("mean_pnl_per_trade"),
#         stdev_pnl_per_trade=result.get("stdev_pnl_per_trade"),
#         sharpe_like_per_trade=result.get("sharpe_like_per_trade"),
#         max_drawdown_pct=result.get("max_dd_pct"),
#         max_drawdown_abs=result.get("max_dd_abs"),
#     )
#
#     notes = result.get("notes", "")
#     csv_note = result.get("csv_note")
#
#     return render_text_report(metrics, exposure, notes, csv_note=csv_note)
...
#         f.write(csv_bytes)
#
# def send_email_with_text_alt(html: str, text_alt: str, csv_bytes: bytes):
...
#     alt.attach(MIMEText(text_alt, "plain"))
...
def main(argv=None):
    ...
    ap.add_argument("--window-hours", type=int, default=RCFG.lookback_hours,
                    help=f"Lookback window in hours (default from env REPORT_LOOKBACK_HOURS={RCFG.lookback_hours})")
    ap.add_argument("--source", default="report_trades",
                    help="Data source label to print in the report header (e.g., 'report_trades')")
    ap.add_argument("--starting-equity", type=float, default=RCFG.starting_equity,
                    help=f"Starting equity used for drawdown math (default from env STARTING_EQUITY_USD={RCFG.starting_equity})")
    ap.add_argument("--csv-note", default=None,
                    help="Optional note to include at the bottom (e.g., saved CSV path)")
    ap.add_argument("--no-email", action="store_true",
                    help="Print the text report to stdout instead of emailing")
    # Optional explicit email overrides; env still works via botreport.emailer
    ap.add_argument("--sender", default=None, help="Override email sender (else REPORT_SENDER / EMAIL_SENDER)")
    ap.add_argument("--to", action="append", dest="recipients",
                    help="Override recipient(s); can repeat or comma-separate (else REPORT_RECIPIENTS / EMAIL_TO)")
    args = ap.parse_args(argv)

    # 1) Resolve time window (UTC)
    start_ts, end_ts = resolve_time_window(args.window_hours)

    # 2) Load raw data from your DB/ORM layer
    # Replace this stub with your actual loader; expected keys shown below.
    raw = load_windowed_data(start_ts, end_ts, args.source)

    # 3) Build the bundle, then compute metrics
    bundle = assemble_bundle(raw, f"last {args.window_hours}h", args.source, args.starting_equity, args.csv_note)

    # carry headline pnl (if you computed earlier)
    bundle.metrics.realized_pnl  = raw.get("realized_pnl")
    bundle.metrics.unrealized_pnl = raw.get("unrealized_pnl")
    bundle = compute_metrics(bundle,
                             raw["closed_trade_pnls"],
                             raw["wins"],
                             raw["losses"],
                             raw["breakevens"]
                             )

    # 4) Render text (canonical for both console and email plain-text)
    text_body = TextRenderer().render(bundle)

    # (Optional) If you also have HTML and CSV bytes from your HTML/CSV renderers, wire them here:
    html_body = None
    csv_bytes = None
...
def _normalize_recipients(maybe_list):
    if not maybe_list:
        return None
