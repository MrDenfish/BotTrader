...
class TradeBot:
    ...
    def load_bot_components(self):
        ...
        self.database_manager = DatabaseManager(self.utility, self.log_manager, self.ticker_manager,
        ...
        self.trading_strategy = TradingStrategy(self.webhook, self.ticker_manager, self.utility, self.exchange, self.alerts,
                                                self.log_manager, self.ccxt_exceptions, self.market_metrics, self.app_config)

        self.profit_helper = ProfitHelper(self.utility, self.portfolio_manager, self.ticker_manager, self.log_manager,
                                          self.app_config)

        self.order_manager = OrderManager(self.trading_strategy, self.exchange, self.webhook, self.utility, self.alerts,
                                          self.log_manager, self.ccxt_exceptions, self.profit_helper, self.app_config)
        ...
        self.profit_manager = ProfitabilityManager(self.exchange, self.ccxt_exceptions, self.utility, self.database_manager,
                                                   self.order_manager, self.portfolio_manager, self.profit_helper,
    ...
    def main(self, port):  # async

        profit_data = pd.DataFrame(columns=['Symbol', 'Unrealized PCT', 'Profit/Loss', 'Total Cost', 'Current Value',
                                            'Balance'])
        self.ticker_cache, self.market_cache = self.database_manager.initialize_db()  # await
        ...
        # with aiohttp.ClientSession() as session:  # async
        try:
            while True:
                print(f'          '
                ...
                self.trading_strategy.set_trade_parameters(start_time,  self.ticker_cache, self.market_cache,
                                                           holdings)  # start_time, session,  self.ticker_cache,
                # self.market_cache, holdings

                self.order_manager.set_trade_parameters(start_time, self.ticker_cache, self.market_cache,
                                                        web_url, holdings)  # start_time, session, self.ticker_cache,
                # self.market_cache, web_url, holdings

                self.profit_manager.set_trade_parameters(start_time, self.ticker_cache, self.market_cache,
                                                         web_url, holdings)  # start_time, session, self.ticker_cache,
                # self.market_cache, web_url, holdings

                self.profit_helper.set_trade_parameters(start_time, self.ticker_cache, self.market_cache,
                                                        web_url, holdings)  # start_time, session, self.ticker_cache,
                # self.market_cache, web_url, holdings
                ...
                self.webhook.set_trade_parameters(start_time, self.ticker_cache, self.market_cache, web_url,
                                                  holdings)  # start_time, session, self.ticker_cache, self.market_cache,
                # web_url, holdings
                ...
                (holdings, usd_coins, avg_vol_total, buy_sell_matrix, price_change) = \
                    (self.api_wrapper.get_portfolio_data(start_time, self.holdings))
                ...
                if filtered_ticker_cache is not None and not filtered_ticker_cache.empty:
                    open_orders, bollinger_df = self.market_manager.new_fetch_ohlcv(  # await
                ...
                intro_text = (f"{len(filtered_matrix)} Currencies trading with a buy or sell signal and Volume greater "
                              f"than"
                ...
                self.profit_manager.check_profit_level(holdings)  # await
                ...
                self.ticker_cache, self.market_cache = self.ticker_manager.update_ticker_cache(start_time)  # await
                # self.database_manager.update_trades_db(self.market_cache)  # await
