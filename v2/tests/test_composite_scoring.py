...
# ------------------------------------------------------------------
# Regime ATR Percentile Lookback
# ------------------------------------------------------------------

class TestRegimeATRPercentile:
    """Verify ATR percentile uses the configurable lookback window."""

    def test_atr_percentile_uses_lookback(self):
        """Prior high-vol history dilutes ranking with long lookback.

        Data: 80 bars very-volatile (range ~500) → 10 bars calm (range ~20)
        → 10 bars moderate (range ~100).

        Short lookback (20): sees only calm + moderate → current ranks HIGH.
        Long lookback (100): sees prior very-volatile bars → current ranks LOW.
        """
        n_hot, n_calm, n_mod = 80, 10, 10
        n_total = n_hot + n_calm + n_mod

        timestamps = [datetime(2026, 1, 1) + timedelta(minutes=i) for i in range(n_total)]

        # Build price series with controlled volatility phases
        base = 50000.0
        closes = np.full(n_total, base)
        highs = np.empty(n_total)
        lows = np.empty(n_total)

        # Phase 1: very volatile (high-low = 500)
        highs[:n_hot] = base + 250
        lows[:n_hot] = base - 250

        # Phase 2: calm (high-low = 20)
        highs[n_hot:n_hot + n_calm] = base + 10
        lows[n_hot:n_hot + n_calm] = base - 10

        # Phase 3: moderate (high-low = 100)
        highs[n_hot + n_calm:] = base + 50
        lows[n_hot + n_calm:] = base - 50
        ...
        # Short lookback: only sees calm + moderate → moderate ATR ranks high
        cfg_short = CompositeScoreConfig(regime_atr_lookback=20)
        ind_short = compute_indicators(df, cfg_short)
        pctl_short = ind_short["_atr_percentile"]

        # Long lookback: includes 80 very-volatile bars → moderate ATR ranks low
        cfg_long = CompositeScoreConfig(regime_atr_lookback=100)
        ind_long = compute_indicators(df, cfg_long)
        pctl_long = ind_long["_atr_percentile"]

        assert pctl_short > 60, f"Short lookback percentile {pctl_short} should be > 60"
        assert pctl_long < 30, f"Long lookback percentile {pctl_long} should be < 30"
        assert pctl_short > pctl_long

    def test_regime_filter_blocks_high_vol_buy(self):
        """Regime filter with short lookback blocks buys during a volatility spike."""
        from unittest.mock import patch

        registry.discover_plugins()

        # 40 bars calm → 20 bars extreme spike
        n_calm, n_spike = 40, 20
        n_total = n_calm + n_spike
        base_ts = datetime(2026, 1, 1)
        base_price = 50000.0

        closes = np.full(n_total, base_price)
        highs = np.empty(n_total)
        lows = np.empty(n_total)

        # Calm phase: tight range
        highs[:n_calm] = base_price + 10
        lows[:n_calm] = base_price - 10
        # Spike phase: wide range
        highs[n_calm:] = base_price + 300
        lows[n_calm:] = base_price - 300

        bus = EventBus()
        s = registry.create_strategy("composite_scoring", event_bus=bus)
        s.configure({
            "regime_filter_enabled": True,
            "regime_max_atr_percentile": 60.0,
            "regime_atr_lookback": 20,
            "min_bars": 40,
        })
        ...
        # Now force a strong buy signal — regime filter should still block it
        last_candle = Candle(
            symbol=symbol,
            timestamp=base_ts + timedelta(minutes=n_total),
            open=base_price,
            high=base_price + 300,
            low=base_price - 300,
            close=base_price,
        ...
        with patch.object(s._guardrails, "apply",
                          return_value=((1, 6.0, 5.5), (0, 0.0, 5.5), "buy", None)):
            result = s.on_candle(last_candle, {})

        # Regime filter should block the buy (return None)
        assert result is None, "Regime filter should block buy during high-vol spike"
